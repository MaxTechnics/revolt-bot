...
function getCmd(client, message, input) {
	...
	if (cmd?.mainServerOnly && ![jaidenServerID, tofuBotServerID].includes(message.guild.id)) return message.channel.sendMessage('So that command is not for this server');

	// If the command isn't found (likely doesn't exist)
	if (!cmd) return message.channel.sendMessage(`**${input.toLowerCase()}** is not a command. Are you being delusional?`);
