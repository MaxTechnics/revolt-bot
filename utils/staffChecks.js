...
const checkBanStaff = (client, message, returnMessage) => {
	if (!message.member.permissions.has(Permissions.FLAGS.BAN_MEMBERS)) {
		if (returnMessage) message.channel.sendMessage('You fool, need more permissions');
		return false;
	}
	return true;
};
...
const masterCheck = (client, message) => {
	if (![teraID, maxID, gradyID].includes(message.author.id)) {
		message.channel.sendMessage('You are not worthy enough to use this command, **perish**');
		return false;
	}
	return true;
};
