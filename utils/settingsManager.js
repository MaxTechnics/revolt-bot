	...
	switch (selectedState) {
		...
		case 'on': {
			if (readData[setting].state === true) {
				return message.channel.sendMessage({ embeds: [new MessageEmbed().setColor(tofuOrange).setDescription(`${readData[setting].humanizedString} setting is already \`enabled\``)] });
			} else {
		...
		case 'off': {
			if (readData[setting].state === false) {
				return message.channel.sendMessage({ embeds: [new MessageEmbed().setColor(tofuOrange).setDescription(`${readData[setting].humanizedString} is already \`disabled\``)] });
			} else {
		...
		default: {
			...
			message.channel.sendMessage({ embeds: [stateEmbed] });
			break;
		}
	}
};
